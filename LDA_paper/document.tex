...
%and it guarantees a bounded model error.
...
be caused by a system fault (e.g. sensor failure) or by a transition in the target concept.
Classical examples of a prediction problem that involves the target concept change are user preference prediction
and fraud detection. In the former, the choices of the user can change over time; in the latter, the fraudulent
transactions change constantly to avoid detection. In both, the target concept changes over time, and if not
detected, renders the prediction model inadequate.

Methods that detect changes in data over time are referred to as {\em concept drift detection} or {\em change-point detection}. A large volume of work has been done on these topics 
(see e.g. ~\cite{basseville1993detection,brodsky2013nonparametric,ChenGupta2000,baena2006early,klinkenberg2000detecting,
wang2015concept,gama2004learning,dries2009adaptive,wang2015concept,icml2014c2_harel14,Nishida2007} for in-depth survey of the field). However, the problem that we address here has not been approached yet and it is distinct from previous work in the following important aspects:
\begin{description}
\item[Concept Change Detection:] The generating distribution of the data could change due to various reasons. Some of them will affect the prediction model and some of them will not, even though these changes could be detected as a distribution switch (by change-point methods \cite{basseville1993detection,brodsky2013nonparametric,ChenGupta2000}). In this work, we are only concerned with the changes of the model itself, while most of the previous work do not make this distinction.
%We address target concept change detection - look for the change that is correlated with the prediction problem as oppose to any change in the generating distribution (a statistical problem)
\item[Model-Based Monitoring] In this work we focus on monitoring the classification models. Most previous work
 on concept drift detection~\cite{baena2006early,gama2004learning,Nishida2007}, focusing  on classification settings, are error-based
 (they utilize classification error rates to draw conclusion about the change of the distribution).
 Here, we propose to monitor the change in the {\em model} as opposed to errors in classification.
 Monitoring the  model and not the errors has an important benefit, which is detecting the drift before
 the prediction error occurs.

\item[Distributed Setting]
    We are the first to look at the concept drift detection in streaming data in distributed setting.
    We assume that the model is learned centralized after all nodes synchronize their data. This process requires
    a lot of resources and communication. Thus the goal is to reduce the number of such synchronisation.
    If the system performs synchronization and retraining periodically, then it can either miss the moment of the
    target change (which leads to erroneous predictions) or performs redundant synchronizations (when the model
    is still valid), resulting in unnecessary communication costs.  To resolve this problem, we define local
    constraints, that if satisfied, guarantee the validity of the global prediction model.
\end{description}


Similarly to~\cite{icml2014c2_harel14}, we link the concept drift detection method with the hypothesis class
and the training algorithm (The former is obvious, the latter is not. Designing a more general
method for concept drift detection that is independent of the training algorithm of the classifier
is left for future work). In this paper we focus on linear binary classifiers as a hypothesis class, and
Linear Fisher Discriminant \cite{fisher1936use} as the learning algorithm.

Linear classifiers well analysed, which allows to base new schemes on them more smoothly.
However, linear classifiers are also very popular in real applications and serve as a platform to more
complex classifiers, such as ensemble models \cite{Deva, eSVM},
...
The first --- Distributed LDA Monitoring (DLDA) -- will sync in a round that 
at least one node has reported a violation in it as it detailed in Alg.
\ref{DLDA}.
The second --- Probabilistic Distributed LDA Monitoring (PDLDA) -- will sync in
